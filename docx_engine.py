...
def fill_doc1_supply_contract(d: dict) -> Document:
    ...
    gen_addr = d.get("발전기주소", "").strip() or d.get("사업장주소", "")
        ...
        for row in table.rows:
            ...
            for i, cell in enumerate(cells):
                ...
                if ct in ("상호", "상 호") and i + 1 < len(cells):
                    ...
                    # ★ 무조건 덮어쓰기 (빈칸 조건 제거)
                    _set_cell_text(cells[i + 1], gen_addr, force_no_bold=True)
                elif ("발 전 기" in ct and "주 소" in ct) and i + 1 < len(cells):
                    # ★ 무조건 덮어쓰기
                    _set_cell_text(cells[i + 1], gen_addr, force_no_bold=True)
...
def fill_doc2_application(d: dict) -> Document:
    ...
    field_map = {
        ...
        "②대표자명":     d.get("대표자명", ""),
        "대 표 자 명":   d.get("대표자명", ""),
        "③이 용 장 소":  d.get("발전기주소", "").strip() or d.get("사업장주소", ""),
        "③이용장소":     d.get("발전기주소", "").strip() or d.get("사업장주소", ""),
        "이 용 장 소":   d.get("발전기주소", "").strip() or d.get("사업장주소", ""),
        "⑥전 화 번 호":  d.get("연락처", ""),
        "⑥전화번호":     d.get("연락처", ""),
        "⑧전자우편주소": d.get("이메일", ""),
